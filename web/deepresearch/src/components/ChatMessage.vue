<template>
    <div :class="messageContainerClass">
      ...
      <div v-if="message.sender === 'bot'" class="flex-shrink-0 ml-2">
  ...
  const messageBubbleClass = computed(() => {
    const baseClasses = ['rounded-lg', 'p-3', 'max-w-xs', 'lg:max-w-md', 'shadow-sm']
    ...
    if (isUser.value) {
      return [...baseClasses, 'bg-blue-500', 'text-white', 'mr-2']
    } else {
